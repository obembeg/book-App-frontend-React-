...
function Book() {
  ...
  const [readPreview, setReadPreview] = useState(true);
          ...
          <div className="md:w-1/2 p-8 sm:p-10 flex flex-col justify-between">
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-2 text-2xl font-extrabold text-white">
                ...
                <span>
                  ₦
                  {books?.price && books.price !== "0" && books.price !== "0.00"
                    ? Number(books.price).toLocaleString()
                    : " Free"}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <button className="flex-1 sm:flex-none px-4 py-2.5 flex items-center justify-center gap-1.5 bg-slate-950 hover:bg-slate-900 border border-slate-800 text-slate-400 hover:text-indigo-500 text-xs font-bold rounded-xl transition duration-200 active:scale-95 cursor-pointer"
                onClick={() => {
                  setReadPreview(!readPreview);
                }}>
                  {readPreview ? <> <IoReaderOutline className="w-4 h-4" />  Read Preview</> : "Coming Soon...🚀"}
                </button>
              </div>
              ...
              <div className="space-y-2 pt-4 border-t border-slate-850">
                ...
                <p className="text-slate-300 text-sm leading-relaxed whitespace-pre-line max-h-40 overflow-y-auto pr-2">
                  {books?.description ||
                    "No plot synopsis was uploaded for this publication."}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-xs pt-6 border-t border-slate-850 mt-6">
              <div className="space-y-1">
                <span className="block text-slate-500 uppercase font-bold tracking-wider text-[10px]">
                  Publisher
                </span>
                <span className="font-semibold text-slate-300">
                  {books?.publisher || "Independent"}
                </span>
              </div>
              <div className="space-y-1">
                <span className="block text-slate-500 uppercase font-bold tracking-wider text-[10px]">
